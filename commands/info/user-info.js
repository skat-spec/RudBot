...
const { formatDate, timer } = require("../../utils/functions");
const ms = require('ms')
const { emoji } = require('../../config.json')
const { badges, ActivityType } = require('../../data/user-info')
...
module.exports ={
    ...
    data.push(`**${emoji[member.presence.clientStatus.web]}** Сайт`);
    }
    if (member.presence.clientStatus.mobile) {
      data.push(`**${emoji[member.presence.clientStatus.mobile]}** Телефон`);
      }
      if (member.presence.clientStatus.desktop) {
        data.push(`**${emoji[member.presence.clientStatus.desktop]}** Компьютер`);
        }
      }
...
ID: **${id}**
Статус: ${data} ${status}
Значки: ${member.user.flags ? member.user.flags.toArray().map(flag => badges[flag]).join(', ') : '**Отсутствуют**'}
