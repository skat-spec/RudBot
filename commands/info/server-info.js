//А вот мой говнокод сервер инфо

const { MessageEmbed } = require('discord.js');
const { emoji } = require('../../config.json')
const { formatDate, timer, time } = require("../../utils/functions");
const { regions, verification, features } = require('../../data/server-info')
const ms = require('ms')
...
Вы присоединились: **${formatDate(new Date(message.guild.member(argsUser).joinedTimestamp))}**\n(**${timer(parseInt(ms(diff2).match(/\d+/)), ['день', 'дня', 'дней'])} назад**)`)    //Дата создания и захода на сервер... Я даже не знаю что сказать     
        .setTimestamp()
        .setColor("303136")
        ...
        embed.addField(`Особенности:`, feat + '\nЕсли вместо особенности "undefined", то пишите создателю бота', true)
    }
    msg.edit('', embed)
}
}